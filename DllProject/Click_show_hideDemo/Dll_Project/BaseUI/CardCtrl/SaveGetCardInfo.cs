...
namespace Dll_Project.BaseUI.CardCtrl
{
    ...
    {
        public string Sendurl = "http://192.168.2.110:3001/expo/api/send_info";
        ...
        private void ToggleClick(bool isOn)
        {
            if (isOn)
            {
                ...
                messagePanel.transform.parent.parent.GetComponent<Canvas>().sortingOrder = 11;
        ...
        private void ShowPersonInfoClick()
        {
            ...
            while (iter.MoveNext())
            {
                ...
                //go.transform.Find("CancelButton").GetComponent<Button>().onClick.AddListener(() => { CancelClick(go); });
                //go.transform.Find("AddButton").GetComponent<Button>().onClick.AddListener(() => { AddCardClick(go); });
            }
            iter.Dispose();
        }
