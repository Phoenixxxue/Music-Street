...
namespace Dll_Project.BaseUI.MyInformation
{
    ...
    public class MyInformationPanelControl : DllGenerateBase
    {
        //public string url = "http://192.168.2.57:3001/api/check_user_detail_info";
        //public string saveurl = "http://192.168.2.57:3001/api/save_user_detail_info";

        public string url = "https://s.vscloud.vip/expo/api/check_user_detail_info";
        public string saveurl = "https://s.vscloud.vip/expo/api/save_user_detail_info";
        ...
        public override void Start()
        {
            ...
            {
                //BaseMono.StartCoroutine(IsHaveTwo());
                //面板数据获取
            ...
            if (mStaticThings.I.now_ScenePrefix == "i")
            {
                nameInput.onEndEdit.AddListener(nameInputFieldEnd);
                enterpriseInput.onEndEdit.AddListener(companyInputFieldEnd);
            }
            else
            {
        ...
        /// </summary>
        /// <returns></returns>
        private IEnumerator IsHaveTwo()
        {
            WWWForm wwwForm = new WWWForm();
            wwwForm.AddField("uid", mStaticThings.I.mAvatarID);
            UnityWebRequest uwr = UnityWebRequest.Post(url, wwwForm);
            yield return uwr.SendWebRequest();
            JsonData jd = JsonMapper.ToObject(uwr.downloadHandler.text);
            if (!string.IsNullOrWhiteSpace(jd["hasInfo"].ToString()))
            {
                if (int.Parse(jd["hasInfo"].ToString()) == 1)
                {
                    if (jd["data"].ToString() != null)
                    {
                        if (!string.IsNullOrWhiteSpace(jd["data"]["company_name"].ToString()))
                            enterpriseInput.text = jd["data"]["company_name"].ToString();
                        if (!string.IsNullOrWhiteSpace(jd["data"]["name"].ToString()))
                            nameInput.text = jd["data"]["name"].ToString();
                        if (!string.IsNullOrWhiteSpace(jd["data"]["position"].ToString()))
                            postInput.text = jd["data"]["position"].ToString();
                        if (!string.IsNullOrWhiteSpace(jd["data"]["contact_info"].ToString()))
                            contactInput.text = jd["data"]["contact_info"].ToString();
                        ...
                        mStaticData.AvatorData.uid = mStaticThings.I.mAvatarID;
        ...
        private static string URL = "http://121.37.129.57/middle";
        private string GetTokenURL = URL + "/api/check";//身份验证
        private string GetCardURL = URL + "/api/get/usercard";//获取我的面板信息
        private string SaveCardURL = URL + "/api/update/usercard";//保存我的面板信息
        private string GetIdentityURL = URL + "/api/identity/list";//获取身份列表
        ...
        {
            //BaseMono.StartCoroutine(IsHave());
            BaseMono.StartCoroutine(GetMyInfo());
        }

        private IEnumerator IsHave()
        {
            WWWForm wwwForm = new WWWForm();
            wwwForm.AddField("uid", mStaticThings.I.mAvatarID);
            UnityWebRequest uwr = UnityWebRequest.Post(url, wwwForm);
        ...
        {
            //BaseMono.StartCoroutine(SaveInfo());
            BaseMono.StartCoroutine(SaveMyInfo());
        }

        private IEnumerator SaveInfomation()
        {
            WWWForm wwwForm = new WWWForm();
            if (string.IsNullOrEmpty(enterpriseInput.text) || string.IsNullOrEmpty(nameInput.text) || string.IsNullOrEmpty(postInput.text) || string.IsNullOrEmpty(contactInput.text))
            {
            ...
            else
            {
                if (!IsEmail(contactInput.text) || !IsPhoton(contactInput.text))
                {
                    wwwForm.AddField("company_name", enterpriseInput.text);
                    wwwForm.AddField("name", nameInput.text);
                    wwwForm.AddField("position", postInput.text);
                    wwwForm.AddField("contact_info", contactInput.text);
                    wwwForm.AddField("uid", mStaticThings.I.mAvatarID);
                    UnityWebRequest uwr = UnityWebRequest.Post(saveurl, wwwForm);
                    yield return uwr.SendWebRequest();
        ...
        {
            name = FilterEmoji(name);//屏蔽emoji
            if (name.Length > 6)
            {
                nameInput.text = name.Substring(0, 6);
                NamePlateText.text = $"{nameInput.text}\n{enterpriseInput.text}";
            }
            else
            {
        ...
        private void companyInputFieldChange(string name)
        {
            ...
            if (name.Length > 12)
            {
                enterpriseInput.text = name.Substring(0, 12);
                NamePlateText.text = $"{nameInput.text}\n{enterpriseInput.text}";
            }
            else
            {
                enterpriseInput.text = name;
                NamePlateText.text = $"{nameInput.text}\n{name}";
            }
        }
        private void nameInputFieldEnd(string name)
        {
            name = FilterEmoji(name);//屏蔽emoji
        ...
        private void companyInputFieldEnd(string name)
        {
