...
namespace Neuron
{
	public class NeuronTransformsInstance : NeuronInstance
	{
		...
		public TransInfoOfBones temp ; // 用于存储每一帧所有骨骼的位置、角度信息
		new void Awake()
        {
			temp = new TransInfoOfBones();
		...
		void SetPosition( Transform[] transforms, NeuronBones bone, Vector3 pos, bool withOriginlPosition = false )
		{
			...
			if( t != null )
			{
				...
				{
					t.localPosition = pos;
					temp.locX[(int)bone] = pos.x;
					temp.locY[(int)bone] = pos.y;
					temp.locZ[(int)bone] = pos.z;
				}
			}
		}
		
		// set rotation for bone
		void SetRotation( Transform[] transforms, NeuronBones bone, Vector3 rotation )
		{
			Transform t = transforms[(int)bone];
			if( t != null )
			{
				...
				{
					t.localRotation = rot;
					temp.rotX[(int)bone] = rot.x;
					temp.rotY[(int)bone] = rot.y;
					temp.rotZ[(int)bone] = rot.z;
					temp.rotA[(int)bone] = rot.w;
				}
			}
		}
		...
		public void ApplyMotion( NeuronActor actor, Transform[] transforms, Vector3[] bonePositionOffsets, Vector3[] boneRotationOffsets , bool enableHipMove,  Quaternion[] orignalRot , Quaternion[] orignalParentRot , Vector3[] orignalPositions)
		{
            if (!actor.HsReceivedData)
                return;
			// apply Hips position
			temp.HandleID = actor.AvatarName + actor.AvatarIndex;
